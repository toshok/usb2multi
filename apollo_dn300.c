...
#define UART_KBD_RX_GPIO 5

// #define UART_MOUSE_ID uart0
// #define UART_MOUSE_TX_GPIO 0
...
void apollo_dn300_init() {
    printf ("in apollo_dn300_init\r\n");
    gpio_set_function(UART_KBD_TX_GPIO, GPIO_FUNC_UART);
    gpio_set_function(UART_KBD_RX_GPIO, GPIO_FUNC_UART);
...
void apollo_dn300_kbd_report(hid_keyboard_report_t const *report) {
    printf("in apollo_dn300_kbd_report\r\n");
	...
	for (int i = 0; i < down_key_count; i++) {
		...
		if (kbd_mode == Mode0_Compatibility) {
			...
			if (code != 0) {
				...
				uart_putc_raw(UART_KBD_ID, (char)code);
				printf(" + sent %04x\r\n", code);
			}
		} else {
			printf("uh...\r\n");
